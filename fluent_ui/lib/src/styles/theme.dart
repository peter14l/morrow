...
const standardCurve = Curves.easeInOutCubic;

/// The animation curve used for elements entering the screen.
const entranceCurve = Cubic(0, 0, 0, 1);

/// The animation curve used for elements exiting the screen.
const exitCurve = Cubic(1, 0, 1, 1);

/// A collection of standard shadows used in the Fluent Design System.
///
/// These shadows are based on WinUI 3 guidelines to convey depth and hierarchy.
@immutable
class FluentShadows with Diagnosticable {
  /// A subtle shadow for small elements like buttons.
  final List<BoxShadow>? elevation2;

  /// A more prominent shadow for medium elements like cards.
  final List<BoxShadow>? elevation8;

  /// A deep shadow for large elements like flyouts.
  final List<BoxShadow>? elevation32;

  /// The deepest shadow for top-level elements like dialogs.
  ...
  factory FluentShadows.fromBrightness(Brightness brightness) {
  ...
  static FluentShadows lerp(FluentShadows? a, FluentShadows? b, double t) {
    return FluentShadows(
      elevation2: BoxShadow.lerpList(a?.elevation2, b?.elevation2, t),
      elevation8: BoxShadow.lerpList(a?.elevation8, b?.elevation8, t),
      elevation32: BoxShadow.lerpList(a?.elevation32, b?.elevation32, t),
      elevation64: BoxShadow.lerpList(a?.elevation64, b?.elevation64, t),
    );
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<BoxShadow>('elevation2', elevation2));
    properties.add(IterableProperty<BoxShadow>('elevation8', elevation8));
    properties.add(IterableProperty<BoxShadow>('elevation32', elevation32));
    properties.add(IterableProperty<BoxShadow>('elevation64', elevation64));
  }
}
...
class FluentThemeData with Diagnosticable {
  ...
  /// Standard shadows for various elevations.
  final FluentShadows shadows;
  ...
  /// The default background color for [MicaAlt] widgets.
  final Color micaAltBackgroundColor;
  ...
  ///
  /// This is used by materials like [Mica] and [Acrylic] to adjust their
  /// appearance when the window loses focus, matching WinUI 3 behavior.
  ///
  /// Defaults to true.
  ...
  factory FluentThemeData({
    ...
    Color? micaAltBackgroundColor,
    Color? shadowColor,
    FluentShadows? shadows,
    Color? menuColor,
    Color? cardColor,
    ...
    shadows ??= FluentShadows.fromBrightness(brightness);
    ...
    micaAltBackgroundColor ??= resources.solidBackgroundFillColorTertiary;
    ...
    windowFocused ??= true;
    // Set Segoe UI Variable as default for Windows, fallback to Segoe UI
    final defaultFontFamily = defaultTargetPlatform == TargetPlatform.windows
        ? 'Segoe UI Variable'
        : 'Segoe UI';
    typography = Typography.fromBrightness(
      brightness: brightness,
      color: resources.textFillColorPrimary,
    ).merge(typography).apply(fontFamily: fontFamily ?? defaultFontFamily);
    ...
    return FluentThemeData.raw(
      ...
      micaAltBackgroundColor: micaAltBackgroundColor,
  ...
  const FluentThemeData.raw({
    ...
    required this.micaAltBackgroundColor,
  ...
  static FluentThemeData lerp(FluentThemeData a, FluentThemeData b, double t) {
    return FluentThemeData.raw(
      ...
      micaAltBackgroundColor: Color.lerp(
        a.micaAltBackgroundColor,
        b.micaAltBackgroundColor,
        t,
      )!,
      shadowColor: Color.lerp(a.shadowColor, b.shadowColor, t)!,
      shadows: FluentShadows.lerp(a.shadows, b.shadows, t),
      ...
      windowFocused: t < 0.5 ? a.windowFocused : b.windowFocused,
    );
  }
  ...
  FluentThemeData copyWith({
    ...
    return FluentThemeData.raw(
      ...
      micaAltBackgroundColor:
          micaAltBackgroundColor ?? this.micaAltBackgroundColor,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
      ...
      ..add(DiagnosticsProperty<FluentShadows>('shadows', shadows))
      ...
      ..add(ColorProperty('micaAltBackgroundColor', micaAltBackgroundColor))
      ...
      ..add(DiagnosticsProperty<bool>('windowFocused', windowFocused))
